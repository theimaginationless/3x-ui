...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/theimaginationless/3x-ui-ext/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/theimaginationless/3x-ui-ext/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/theimaginationless/3x-ui-ext/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/theimaginationless/3x-ui-ext/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/theimaginationless/3x-ui-ext/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
